...
// NEW: Single Chart Mode for the Modal
if (isset($input['mode']) && $input['mode'] === 'chart') {
    $chart = KPUtils::getBatchKPChart($startTimestamp, 1, $lat, $lon, $sidMode, $SWETEST_PATH, $EPHE_PATH);
    $asc = $chart[0]['houses'][1];
    $lords = KPUtils::getKPLords($asc);
    
    echo json_encode([
        "chart" => $chart[0] ?? null,
        "lords" => $lords,
        "time" => date('Y-m-d H:i:s', $startTimestamp),
        "input" => $input
    ]);
    exit;
}
